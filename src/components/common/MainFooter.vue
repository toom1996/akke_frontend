    ...
    <div class="wrapper footer-language group/languages">
      <div class="border-r border-slate-950 flex items-center justify-center">
        <button type="button" class="text-black px-2 text-center inline-flex items-center">
          <Icon :name=state.currentLanguage.flag />
          {{ state.currentLanguage.str }}
        </button>
        <div class="tooltip group-hover/languages:block ">
          ...
          <ul class="flex flex-col-reverse">
            <li v-for="item in state.dataProvider.languages" class="LanguageSelector-footerSelectorItem-m2u" role="menuitem">
...
import FlagChina from "@/assets/icons/FlagChina.vue";
...
<style>
footer a {
  @apply px-4 font-bold text-xs text-slate-600 cursor-pointer;
}
...
.tooltip {
  @apply absolute bottom-[calc(100%)] border border-slate-950 py-2 pr-2 bg-white z-50 hidden;
}
